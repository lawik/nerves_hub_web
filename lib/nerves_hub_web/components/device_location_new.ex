defmodule NervesHubWeb.Components.DeviceLocationNew do
  ...
  def render(%{location: location} = assigns) do
    ...
    ~H"""
    <div class="display-box">
      <img class="max-w-[524px]" src={"https://api.mapbox.com/styles/v1/mapbox/satellite-streets-v12/static/#{@lng},#{@lat},#{@zoom},0/#{@size}@2x?access_token=#{@mapbox_access_token}"} />
    </div>
    """
  end
  ...
  defp location_header(assigns) do
    ~H"""
    <div class="text-xs text-base-400"><%= @tooltip %></div>
    """
  end
end
